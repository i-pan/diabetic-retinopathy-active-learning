...
class Strategy:
    ...
    def __init__(self, labeled_dataset, unlabeled_dataset, net, nclasses, args={}):
        ...
        if "loss" not in args:
            ...
            assert args["loss"] == "CrossEntropyLoss", f"Only cross-entropy loss is supported at this time"

        if "activation" not in args:
            self.act_fn = partial(torch.softmax, dim=1)
        else:
            assert args["activation"] == "softmax", f"Only softmax activation is supported at this time"
    ...
    def predict(self, to_predict_dataset):
        ...
        to_predict_dataloader = DataLoader(to_predict_dataset,
        ...
        with torch.no_grad():
            for batch_idx, elements_to_predict in tqdm(enumerate(to_predict_dataloader), total=len(to_predict_dataloader)):
    ...
    def get_embedding(self, to_predict_dataset):
        ...
        with torch.no_grad():
            ...
            for batch_idx, elements_to_predict in tqdm(enumerate(to_predict_dataloader), total=len(to_predict_dataloader)):
                ...
                # Calculate softmax (probabilities) of predictions
                elements_to_predict = elements_to_predict.to(self.device)
                out, l1 = self.model(elements_to_predict, return_features=True)
    ...
    def get_grad_embedding(self, dataset, predict_labels, grad_embedding_type="bias_linear"):
        ...
        self.model = self.model.to(self.device)
        # Turn off gradients for backbone, leave for linear layer
        ...
        if predict_labels:
            for batch_idx, unlabeled_data_batch in tqdm(enumerate(dataloader), total=len(dataloader)):
                start_slice = evaluated_instances
                end_slice = start_slice + unlabeled_data_batch.shape[0]
                
                inputs = unlabeled_data_batch.to(self.device, non_blocking=True)
                out, l1 = self.model(inputs, return_features=True)
                targets = out.max(1)[1]
                
                # Calculate loss as a sum, allowing for the calculation of the gradients
                # using autograd wrt the outputs (bias gradients)
    ...
    def get_feature_embedding(self, dataset, unlabeled):
        dataloader = DataLoader(dataset,
                                batch_size=self.args['batch_size'],
                                shuffle=False,
                                drop_last=False,
        ...
        features = []
        if unlabeled:
            for batch_idx, inputs in tqdm(enumerate(dataloader), total=len(dataloader)):
                inputs = inputs.to(self.device)
                with torch.no_grad():
                    out, batch_features = self.model(inputs, return_features=True)
                features.append(batch_features)
        else:
            for batch_idx, (inputs, _) in tqdm(enumerate(dataloader), total=len(dataloader)):
                inputs = inputs.to(self.device)
                with torch.no_grad():
                    out, batch_features = self.model(inputs, return_features=True)
                features.append(batch_features)
