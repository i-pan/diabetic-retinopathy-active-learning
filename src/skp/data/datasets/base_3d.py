...
class ImageStackDataset(Dataset):
    ...
    Each element in `inputs` should be a DIRECTORY which contains all the images
    ...
    def apply_op(func, data):
        ...
        if "mask" in data:
            ...
            data["mask"] = np.stack([[output["mask"]] + [output[f"mask{i}"] for i in range(1, num_img)]])
        else:
    ...
    def process_image(self, data):
        ...
        if self.resize:
            data = self.apply_op(self.resize, data)
        ...
        if self.crop:
            data = self.apply_op(self.crop, data)
        if self.preprocess:
