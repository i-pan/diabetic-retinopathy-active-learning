...
def build_trainer(cfg, args, snapshot=-1):
    if args.mode == "active_learning":
        version = f"round{cfg.active_learning.round_idx:03d}"
        experiment_name = osp.join(cfg.experiment.name, args.active_learning_strategy)
    else:
    ...
    elif isinstance(cfg.data.get("outer_fold"), (int, float)):
    ...
    if args.group_by_seed:
        if fold_subfolder == "":
            fold_subfolder = f"seed{cfg.experiment.seed}"
        else:
    ...
    group_name = cfg.experiment.name.replace("/", "_")
    if args.mode == "active_learning":
        group_name = group_name + f"_round{cfg.active_learning.round_idx:03d}"
    elif args.mode != "train":
        group_name = args.mode + "_" + group_name
    ...
    return trainer, save_experiment_path
...
def train(cfg, args):
    task = define_task(cfg)
    num_snapshots = cfg.train.get("num_snapshots") or 1
    if num_snapshots == 1:
        trainer, _ = build_trainer(cfg, args)
        trainer.fit(task)
        ...
        for i in range(num_snapshots):
            ...
            trainer, _ = build_trainer(cfg, args, task, snapshot=i)
            trainer.fit(task)
            symlink_best_model_path(trainer)
